...
Run the notebooks in numerical order from `01` to `05`, because later stages depend on outputs generated by earlier notebooks. The first notebook handles the required FoMo data access and preparation, followed by sensor fusion, LiDAR processing, occupancy mapping, and A* path planning.
...
The FoMo recordings were collected using a **Clearpath Warthog UGV** equipped with multimodal sensors for forest navigation research.
...
Project code in this repository is released under the **MIT License**.

The FoMo dataset is not redistributed in this repository and remains subject to its original dataset license.